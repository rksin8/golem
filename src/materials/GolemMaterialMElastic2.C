...
#include "GolemMaterialMElastic2.h"
#include "Assembly.h"
#include "MooseMesh.h"
...
registerMooseObject("GolemApp", GolemMaterialMElastic2);

template <>
InputParameters
validParams<GolemMaterialMElastic2>()
{
  ...
  params.addParam<MooseEnum>("strain_model",
                             GolemMaterialMElastic2::strainModel() = "small_strain",
  ...
  params.addParam<MooseEnum>(
      "permeability_type",
      GolemMaterialMElastic2::permeabilityType() = "isotropic",
...
GolemMaterialMElastic2::GolemMaterialMElastic2(const InputParameters & parameters)
...
MooseEnum
GolemMaterialMElastic2::strainModel()
{
  return MooseEnum("small_strain=1 incr_small_strain=2 finite_strain=3");
}

MooseEnum
GolemMaterialMElastic2::permeabilityType()
{
...
*******************************************************************************/
void
GolemMaterialMElastic2::setPropertiesM()
{
...
void
GolemMaterialMElastic2::setStrainModel()
{
...
void
GolemMaterialMElastic2::setElasticModuli()
{
  ...
  else if (_shear_modulus_set && _bulk_modulus_set)
  {
    ...
    mooseError("Incorrect combination of elastic properties in GolemMaterialMElastic2ech.");
...
void
GolemMaterialMElastic2::setBackgroundStress()
{
...
void
GolemMaterialMElastic2::setPropertiesHM()
{
  if (!isParamValid("permeability_initial") || !isParamValid("fluid_viscosity_initial"))
    mooseError("GolemMaterialMElastic2: You need to provide values for permeability and fluid "
...
void
GolemMaterialMElastic2::setPropertiesTM()
{
  if (!isParamValid("fluid_thermal_conductivity_initial") ||
      !isParamValid("solid_thermal_conductivity_initial"))
    mooseError("GolemMaterialMElastic2: You need to provide values for thermal conductivities for "
  ...
  _alpha_T = getParam<Real>("thermal_expansion_coeff"); // Linear not volumetric
  ...
  if (_has_scaled_properties)
  {
    ...
    _alpha_T  /= _scaling_uo->_s_expansivity;
...
void
GolemMaterialMElastic2::setPropertiesTHM()
{
  // Parameters
...
void
GolemMaterialMElastic2::initQpStatefulProperties()
{
...
void
GolemMaterialMElastic2::computeProperties()
{
  computeStrain();
...
void
GolemMaterialMElastic2::computeStrain()
{
...
void
GolemMaterialMElastic2::computeQpFiniteStrain()
{
...
void
GolemMaterialMElastic2::computeQpProperties()
{
...
void
GolemMaterialMElastic2::GolemCrackClosure()
{
  if (_crack_closure_set)
  {
    std::vector<Real> iso_const(2);
...
void
GolemMaterialMElastic2::GolemMatPropertiesHM()
{
...
void
GolemMaterialMElastic2::GolemKernelPropertiesHM()
{
...
void
GolemMaterialMElastic2::GolemKernelPropertiesDerivativesHM()
{
...
void
GolemMaterialMElastic2::GolemMatPropertiesTM()
{
  _scaling_factor[_qp] = computeQpScaling();
  // Fluid density
  _fluid_density[_qp] = _fluid_density_uo->computeDensity(0.0, 0.0, _rho0_f);
...
void
GolemMaterialMElastic2::GolemKernelPropertiesTM()
{
...
void
GolemMaterialMElastic2::GolemKernelPropertiesDerivativesTM()
{
  ...
  // TODO: instead of bulk_thermal_expansion_coeff use linear one  
  //Real bulk_thermal_expansion_coeff =
  //    _porosity_old[_qp] * _alpha_T_f + (1.0 - _porosity_old[_qp]) * _alpha_T_s;
  (*_TM_jacobian)[_qp] = -_alpha_T * _Cijkl[_qp] *
                         RankTwoTensor(RankTwoTensor::initIdentity);
}

void
GolemMaterialMElastic2::GolemMatPropertiesTHM()
{
  // Fluid density
  Real pres = (*_pf)[_qp];
  Real temp = (*_temp)[_qp];
...
void
GolemMaterialMElastic2::GolemKernelPropertiesTHM()
{
  GolemKernelPropertiesHM();
  GolemKernelPropertiesTM();
...
void
GolemMaterialMElastic2::GolemKernelPropertiesDerivativesTHM()
{
...
unsigned
GolemMaterialMElastic2::nearest()
{
  unsigned n = 0;
...
void
GolemMaterialMElastic2::GolemMatPropertiesM()
{
  // FLuid density
  _fluid_density[_qp] = _fluid_density_uo->computeDensity(0.0, 0.0, _rho0_f);
  // Porosity
...
void
GolemMaterialMElastic2::GolemKernelPropertiesM()
{
...
void
GolemMaterialMElastic2::GolemSubstractEigenStrain()
{
  if (_strain_model > 1)
  {
    _strain_increment[_qp] = _total_strain_increment[_qp];
...
void
GolemMaterialMElastic2::substractThermalEigenStrain(RankTwoTensor & strain_incr)
{
  ...
  if (_has_T)
  {
   ...
   // TODO: instead of bulk_thermal_expansion_coeff use linear one  
    thermal_es_incr.addIa( _alpha_T * ((*_temp)[_qp] - (*_temp_old)[_qp]));
  }
  strain_incr -= thermal_es_incr;
}

void
GolemMaterialMElastic2::GolemStress()
{
  switch (_strain_model)
  {
    case 1:
